 ...
 */
fun diffImage(first: Image, second: Image): Image {
    @Language("GLSL")
    val diffShader = """
        uniform shader a;
        uniform shader b;
        ...
        half4 main(float2 coord) {
            ...
            for(int x = -$size; x < 0; x++) {
                for(int y = -$size; y <= $size; y++){
                    left += content.eval(coord + half2(x, y));
                }
            }
            left /= ($size * $size + 1);
            ...
            for(int x = 1; x <= $size; x++) {
                for(int y = -$size; y <= $size; y++){
                    right += content.eval(coord + half2(x, y));
                }
            }
            right /= ($size * $size + 1);
            ...
            for(int x = -$size; x <= $size; x++){
                for(int y = -$size; y < 0; y++) {
                    top += content.eval(coord + half2(x, y));
                }
            }
            top /= ($size * $size + 1);
            ...
            for(int x = -$size; x <= $size; x++){
                for(int y = 1; y <= $size; y++) {
                    bottom += content.eval(coord + half2(x, y));
                }
            }
            bottom /= ($size * $size + 1);
