...
This list contains a list of known issues/limitations of Compose Hot Reload. The limitations listed here are mainly
caused by the external factors rather than Compose Hot Reload itself. Therefore, we cannot guarantee that these issues
will be resolved in the future. The list is not exhaustive.
...
window can cause weird behaviors (flickering, jumping, etc.) on some window managers. If you have encountered such issues,
please [report](https://github.com/JetBrains/compose-hot-reload/issues) them to us; we will try to reproduce and fix them.

In the meantime, you can try running the dev tools window in the detached mode by setting `-Dcompose.reload.devToolsDetached=true` property.
...
Which is expected: user applications should not rely or care on the current desktop of the window.
...
Thus, currently we recommend running the dev tools window in the detached mode if your workflow includes the use
of virtual desktops. You can enable detached mode by setting `-Dcompose.reload.devToolsDetached=true` property.
...
So for now, we recommend moving your project into an NTFS formatted drive to allow Gradle File Watcher to work.
...
Kotlin Multiplatform IDE plugin currently only supports a single Compose Hot Reload connection at a time. If you try to 
simultaneously run multiple Compose applications within the same IDE instance, you may experience unexpected shutdowns.

Single Compose Hot Reload connection at a time in KMP is done rather intentionally, having multiple Compose Hot Reload
related UI elements in the IDE may cause more confusion than help. So if you're running multiple Compose applications 
...
`compose.application.resources.dir` system property from the application. However, Compose Gradle plugin currently sets
this property only for `run` task. For other tasks, including `desktopRun`, `hotRunDesktop`, etc., this property is not set.
