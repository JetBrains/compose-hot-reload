...
import org.jetbrains.compose.reload.logging.HotReloadLoggerService
...
internal class HotReloadOrchestrationLoggerService : HotReloadLoggerService {
    override fun getLogger(name: String, tag: String?): HotReloadLogger {
        ...
        return logger.with(createLogger(name) { message ->
            orchestration?.sendMessage(
                OrchestrationMessage.LogMessage(tag, message)
            )
        })
    }
}
