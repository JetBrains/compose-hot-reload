...
open class CheckScreenshotBenchmark {
    ...
    @Setup
    fun generateImages() {
        fun generateImage(random: Random, size: Int): BufferedImage {
            val image = BufferedImage(size, size, TYPE_INT_RGB)
            for (x in 0 ..< size) {
                for (y in 0 ..< size) {
                    val rgb = ((random.nextInt(256) and 0xFF) shl 16) or
        ...
        repeat(numberOfImages) {
            val expected = generateImage(random, imageSize)
            expectedImages += expected
            actualImages += when {
                random.nextBoolean() -> expected
                else -> generateImage(random, imageSize)
            }
        }
    }

    @Benchmark
    fun ssim(blackhole: Blackhole) {
        repeat(numberOfImages) {
            blackhole.consume(ssimDiff(expectedImages[it], actualImages[it]))
        }
    }
